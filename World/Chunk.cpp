...
{
    for ( ARC::UnsignedLong x = 0 ; x < 16 ; ++x )
        for ( ARC::UnsignedLong z = 0 ; z < 16 ; ++z )
            chunk.setBlockType ( ARC::Vector3SignedInt ( x , 0 , z ) , Tortuga::Block::Bedrock ) ;

    for ( ARC::UnsignedLong x = 0 ; x < 16 ; ++x )
        for ( ARC::UnsignedLong z = 0 ; z < 16 ; ++z )
            chunk.setBlockType ( ARC::Vector3SignedInt ( x , 1 , z ) , Tortuga::Block::Stone ) ;

    for ( ARC::UnsignedLong x = 0 ; x < 16 ; ++x )
        for ( ARC::UnsignedLong z = 0 ; z < 16 ; ++z )
            chunk.setBlockType ( ARC::Vector3SignedInt ( x , 2 , z ) , Tortuga::Block::Stone ) ;

    for ( ARC::UnsignedLong x = 0 ; x < 16 ; ++x )
        for ( ARC::UnsignedLong z = 0 ; z < 16 ; ++z )
            chunk.setBlockType ( ARC::Vector3SignedInt ( x , 3 , z ) , Tortuga::Block::Dirt ) ;

    for ( ARC::UnsignedLong x = 0 ; x < 16 ; ++x )
        for ( ARC::UnsignedLong z = 0 ; z < 16 ; ++z )
            chunk.setBlockType ( ARC::Vector3SignedInt ( x , 4 , z ) , Tortuga::Block::Grass ) ;
